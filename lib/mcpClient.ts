...
2. 일본어 말투나 번역 투 금지: '먼저 ~합니다'로 시작하는 일본어 직역투(먼저~)보다는 '낙지는 ~해주세요' 처럼 자연스럽고 구어체적인 한국어 표현을 사용하세요.
...
[필수 지침 - 위반 시 무조건 실패]
1. 일본어(먼저, の, は), 한자(等, 辣, 调節), 영어 등 모든 외국어 및 번역체 말투 엄격 금지.
...
3. 결과는 반드시 JSON 객체 형태로 반환하세요:
   ...
   - advice: 조리 시 유의사항이나 팁 (string)`,
        ...
        {
          role: 'user',
          content: `재료: "${ingredient}", 요리 맥락: "${
            recipeContext || '일반 한국 요리'
          }"`,
        },
      ],
      model: 'llama-3.3-70b-versatile',
